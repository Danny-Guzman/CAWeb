...
jQuery(document).ready(function ($) {
  ...
  function addAlertControls(c) {
    ...
    removeAlert.addEventListener('click', function (e) {
      removeAlertFunc(this);
    });
...
jQuery(document).ready(function ($) {
  ...
  $(window).on('beforeunload', function () {
    if (changeMade && "nav-menus.php" !== caweb_admin_args.changeCheck) return 'Are you sure you want to leave?';
  });
  $('#caweb-options-form select,#caweb-options-form input').on('change', function () {
    changeMade = true;
  });
  $('#caweb-options-form input').on('input', function () {
    changeMade = true;
  });
  $('#caweb-options-form input[type="button"],#caweb-options-form button:not(.doc-sitemap)').on('click', function () {
    changeMade = true;
  });
  ...
  */
  ...
  function addExternal(ext_list, ext) {
    ...
    fileUpload.addEventListener('change', function () {
      ...
      var extension = name.lastIndexOf(".") > 0 ? name.substring(name.lastIndexOf(".") + 1).toLowerCase() : "";
...
(function ($) {
  ...
  $(function () {
    ...
    $(document).on('click', 'div .library-link', function (event) {
      ...
      if (!!types && types.indexOf(',') > 0) types = types.split(',');
      ...
      frame.on('select', function () {
        ...
        if ("true" !== icon_check) {
          if (input_box.length) input_box.val(attachmentURL);
          if (preview_field.length) preview_field.attr('src', attachmentURL);
          if (filename_box.length) filename_box.val(attachmentFileName);
          if (alt_text_box.length) alt_text_box.val(attachmentAlt);
        } else {
          var data = {
...
/******/ })()
;
//# sourceMappingURL=caweb-admin.js.map